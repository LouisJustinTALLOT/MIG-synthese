...
\end{center}
%Est-ce vraiment utile de mettre tous les logos ? On cite déjà les différentes entreprises/universités/labos... --S qui est S
...
\paragraph{} Au bout de quelques jours, le régime stationnaire est atteint et l’équation différentielle admet la solution :
$$
...
Par ailleurs, notons $\text{erfc}$ la fonction d’erreur complémentaire :
$$
\text{erfc}(u)=\frac{2}{\sqrt{\pi}} \int_{u}^{+\infty } e^{-\xi^2} d\xi
$$
Il existe une solution de l’équation en régime transitoire avec $c(0,t)=c_0$, de la forme :
$$
c(z,t)=\frac{c_0}{2} \Big( \exp(-\frac{z}{L}) \, \text{erfc}(\frac{z}{2L\sqrt{ \lambda t}} - \sqrt{\lambda t}) + \exp(\frac{z}{L}) \, \text{erfc}(\frac{z}{2L\sqrt{\lambda t}} + \sqrt{\lambda t} ) )
$$
...
La figure suivante montre l'impact significatif que le coefficient de diffusion a sur la diffusion du radon. Sur le site de la Ribière, on peut l'estimer autour de $1 \; .10^5 \; \text{m}^2 \text{s}^{-1}$.
...
\paragraph{} Les stériles contiennent du radium d’activité massique $A_{Ra,s}$. Le radium se désintègre avec un coefficient $\lambda_{Ra}$. Ajoutons un terme source dans l’équation différentielle en concentration de radon $c(z,t)$ (exprimée en mol/m$^3$) :
$$
...
\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{II_A2_5.png}
