...
\usepackage{eurosym}
\usepackage{hyperref}
%\usepackage{ulem} % comments % A ENLEVER QUAND C'EST FINI
...
\definecolor{red}{rgb}{1.0, 0.0, 0.0} % Nicolas Seigneur
\newcommand{\red}[1]{\textcolor{red}{#1}} % comments
...
\end{itemize}
%et tout particulièrement leurs encadrants du Centre de Géosciences Mines ParisTech - PSL :
%Merci pour votre soutien tout au long de ce MIG !
...
Si l'après-mine est aujourd'hui une problématique prise au sérieux par les exploitants, elle reste souvent mal connue des populations. Bien que de nombreuses solutions soient mises en place, il est encore nécessaire d’informer les citoyens et de donner une plus grande importance à l’après-mine à l'avenir.
\end{abstract}

{\selectlanguage{english}
\begin{abstract}
%France used to be a major mining country. Yet, the exploitation of most of French mines has ended and the mining sites are currently under rehabilitation which constitutes the post-mine process. It aims at addressing environmental and health-related issues (such as the mine waste management), as well as socio-economical (such as finding consensus with local associations and populations) and political (for instance the definition of standards) issues.

France has now numerous mining sites that have ceased all activities and that need to be rehabilitated. This issue is called post-mine. Post-mine aims at addressing environmental and health-related issues (such as the mine waste management), as well as socio-economical (such as finding consensus with local associations and populations) and political (for instance the definition of standards) issues.

This study addresses most of these questions. Its goal is to identify both the different parties involved in the post-mine process and the associated issues, in order to provide some answers. It also deals with technical aspects such as the treatment of contaminated water from mine waste.
%such as water quality management.

We will focus on former uranium mines, on the water treatments and on the alleviation of radon. Special attention is paid to the site of the former La Ribière uranium mine. We thus propose a synthesis of post-mine issues, with a particular focus on the constraints of the different standards and the restrictions imposed by social actors. We also draw a comparison of the different types of existing treatments and an overview of post-mine management in the major mining countries.
%We will particularly study the former La Ribière uranium mine and the possibility of water treatment. After having synthesized these questions, we will draw a comparison between the diverse existing means of treating water and investigate the strategies implemented by the major mining countries to deal with the post-mines problematics.

We acknowledge that the major polluting substances (radon, radium, uranium…) can be efficiently treated.  hydrogeochemical model established for our study also makes it possible to conclude on the case of La Ribière: the evolution of the water quality, as envisaged by it, shows that a treatment does not seem necessary for uranium. However, the inaccuracies of our model prevents us from making a final judgment.
%A hydrogeochemical model of La Ribière site will enable us to determine whether if the treatment of the effluents is necessary.

The post-mine process is a serious issue which is often dealt with by the exploiting firms but quite ignored by the local populations. Although many measures have been enforced, it is still necessary to inform the citizens to bring a wider attention to the post-mine process.
...
%Le règlement R-90 traite de la fermeture et de la réhabilitation des sites.
...
%De plus, il n'est pas rentable de remettre en état les anciens sites en respectant les normes actuelles.
...
%j'enlève le \leftrightharpoons pour uniformiser
...
\begin{figure}[H]
    ...
    \end{minipage}\hfill
\end{figure}
%Les légendes s'arrangent mal. Qqun saurait corriger ça ? @LJ ?
%probleme reglé
...
%paragraphe moins important àmha
...
\subsubsection{Comparaison et faisabilité des méthodes de traitement}
...
En termes de coûts, les traitements par zones humides et par précipitation sont plus avantageux que celui par résine car l’investissement et l’élution sont très coûteux, même en prenant en compte les éventuelles ventes après régénération de la résine. %VL:L’un des principaux enjeux de l’après-mine est de diminuer ces coûts car le traitement par résine est très avantageux lorsque la substance d'intérêt peut être récupérée.
...
%\paragraph{Allemagne \\ \\}
%Lors de la réhabilitation d’une mine d’uranium, la société allemande Wismut GmbH peut procéder à une inondation des mines souterraines, un traitement des eaux contaminées et au stockage des résidus miniers…
...
%\blue{est-ce que l'on répond à la question?}
...
Selon les mesures effectuées par Orano Mining \cite{chautard_evaluation_2020}, le site respecte les normes environnementales en vigueur en termes de qualité des eaux à l’exutoire (le ruisseau du Verraux). La concentration en uranium mesurée est de $7,6 \, . 10^{-6}$~mol/L et la dose mesurée est de $1,37$~Bq/L pour le $^{226}$Ra. En effet, les normes applicables sont de $1,7$~Bq/L pour le radium, par exemple.
%ce qui est inférieur 
%Dernière phrase pas très claire
%Référence des valeurs ? Par ailleurs 1,37 Bq/L=1370 Bq/m^3 ce qui est supérieur à la norme de 300 Bq/m^3 (mais peut être qu'ici c'est une autre norme qui s'applique...
...
%\blue{A noter pour les conclusions générales: c'est une approche importante pour pouvoir anticiper les potentielles évolutions des résidus et stériles miniers et évaluer l'impact environnemental}
...
%par contre pourquoi la figure au dessus est commenté ?
%LJ l'a mise en annexe je crois
%mais en plus on a la place.......surtout quand on parle explicitement dans le texte pour expliquer comment est fait le modèle.... une image c'est toujours mieux et personne va aller se faire chier à regarder en annexe pour ça....
%Et ça change pas le nombre de page. Plus clair comme ça donc c'est mieux
% je sais pas lesquelles j'eai bougé
...
%On obtient donc une charge initiale à une profondeur d'environ 30~mètres, sauf proche de la petite rivière où la charge est situé à seulement quelques mètres.
%\blue{Préciser le gradient de charge, donnez une valeur approx.}
%Un peu redondant non? Je pense qu'on peut carrément virer la deuxième phrase
% +1, ça se répète inutilement
...
%Je pense qu'il faut appeler la réf différemment
...
%\blue{C'est très bien. Il faut ajouter des bases: le sol est un milieu très perméable, donner des ordres de grandeur de perm et poro, $\rightarrow$ les vitesses importantes; le granite est un milieu très peu perméable, donner des ordres de grandeur, l'écoulement est limité dans le granite.}
...
\end{table}
%ça change rien, table les met automatiquement au dessus --S
...
\begin{figure}[H]
    ...
    \caption{Évolution du traceur associé à la zone de résidus après 25 ans de simulation}
...
%\blue{! il faut mettre un exemple: par ex. front de dissolution des argiles localisé qq part si blablabla}
%Pas eu le temps. Il faudrait faire joujou avec le modèle, mais je crois qu'il est un peu tard pour lancer des simulations...
...
%\blue{Cette augmentation de l’acidité s’accompagne d’une légère augmentation des concentrations en métaux (à préciser) restant toutefois inférieurs aux normes (?) (à préciser aussi avec un ex à nouveau pour le Fe, Al, Ba, par ex. Vous pouvez mettre des figures en annexe)} 
%A nouveau, je n'ai pas de données quantifiées de concentrations en métaux  selon les paramètres sur ma machine. Si quelqu'un en a, tant mieux. Sinon, on ne va pas pouvoir trop parler de ça.
%regarde peut-etre dans les résultats de simulation si tu peux avoir la valeur pour Fe, Ba et Al, sinon on ne peut rien dire de plus
...
\begin{figure}[H]   
    \centering
    \subfloat[\centering pH des différentes zones au début de la simulation]{\includegraphics[width=\textwidth]{LJ-pH-initial.PNG} \label{fig:pH-initial} }
    \hfill
    \subfloat[\centering pH des différentes zones après 25 ans]{\includegraphics[width=\textwidth]{LJ-pH-final.PNG} \label{fig:pH-final}}
...
%\blue{C'est déjà beaucoup! combien d'espèces dans le système?}.
...
%Expliquer ce qu'est l'équilibre de Henry ? ou mettre une note pour dire quelle annexe regarder ?
...
%\paragraph{} \blue{Nous vous proposons un plan général \\
%1/ Rappeler quelles sont les enjeux de l'après-mine en général, difficultés techniques (fond geochimique, bruit de fond)\\
%2/ Travail technique sur le site de la Ribière. Axe l'eau, axe gaz. On s'interesse à \underline{l'U mais pas que}\\
	%- l'analyse hydro et chimie du site, traitements, Rn \\
	%- tests de sensibilité, scenario réalisés -> -> conclusions, difficultés, limites techniques \\
%3/ Rappeler les acteurs, la gestion, PNGMDR, difficultés de gestion, de communication ? Nouvelle methode multicritère (IRSN)\\
%- 1. besoin de concertation (respect des objectifs de tous les acteurs) et d’un moyen de concaténer des indicateurs très différents (comment faire une moyenne pondérée entre concentration, coût de dépollution, impact sur une population de batraciens, et besoin de développement touristique) et \\
%- 2. de la difficulté de l’entreprise (norme).\\
%4/ => Vision globale sur la construction des normes en général.  Role de l’exploitant, role de l'état, société. Votre opinion}
...
%2 
% bonsoir irina je suis en train de rajouter les images du mail d'hier avec des petites explications 
% un peu quand même mais ça devrait aller
% notamment l'autunite tout ça je les rajoute 
% Bonsoir ! Très bien =) qu'est-ce qui vous manque ? 
...
% bonsoir. Ca va Guillaume?

%NS: qques endroits où vous pouvez gagner de la place:
% fig 5-6-7: sur une ligne? FAIT
% fig 8-9: ça peut se mettre en annexe FAIT, figure 10-13: peut être possible de condenser. 
% figure 14 et 17: peut peut être supprimer/annexe
% figure 18-21 : annexe (fig 22-23 montrent essentiellemlent la même même mais sont plus synthéthique)
% figure 25: vous pouvez enlever tous les z < 330 et z > 408 et gagner plusieurs cm
% figure 27 & 28: mettez les les unes en dessous des autres avec une seule colobar et une seule caption
% fig 29: mettez la cbar autrement et virez les parties grises, vous gagnerez aussi de la place
% fig 30 & 31: à combiner (une seule caption, une seule colobar, éventuellement côte à côte)
% fig 32-33: cote à cote
%% je dis pas que tout ça est optimal mais ce sont des manières efficaces de gagner sans doute 2-3-4 pages :) 
%SI VOUS APPLIQUEZ CE QUI EST AU DESSUS PENSEZ A METTRE A JOUR LES AUTRES LES NUMEROS DES FIGURES QUI SUIVENT (si vous mettez une figure en annexe/ou la supprimer, diminuez de 1 le numéro des figures suivantes ici)
%Oui, probablement !
%Robin : ok, on se demandait ce qui était compté dans les pages comptabilisées... biblio, annexes, on ne sait pas trop
%Sophian : On peut essayer en priorité d'enlever les espaces perdus inutilement (ce que propose NS pour les figures 27-29-30-31 typiquement). Je pense qu'il faut aussi revoir les parties "comparaison internationale" (si qqun est chaud pour la faire avec moi...)
% coucou sophian Coucou ! :)
% je pexu déplacer les figures 27... 31 hein
%Ce serait top (il faut les refaire non ? plutôt que les déplacer) euh je sais pas 



%4/ => Vision globale sur la construction des normes en général.  Rôle de l’exploitant, rôle de l'état, société. Votre opinion
...
% Non : % si, vous pouvez mettre ce paragraphe à la fin 
...
% @LJ j'ai commenté la ligne au-dessus parce que ça créait une page avec du texte sans aucun sens...
\startcontents[appendices]
\printcontents[appendices]{l}{1}{\section*{Annexes}\setcounter{tocdepth}{1}}
\newpage
\stopcontents[appendices]
\addcontentsline{toc}{section}{Annexes}
\resumecontents[appendices]
\section{Figures et tableaux utiles}
...
\begin{figure}[H]
    ...
    \caption{Chaîne de désintégration de l’uranium (Wikimedia Commons)}
...
\begin{figure}[H]
    ...
    \caption{Comparaison des normes radiologiques sur le radon (IRSN)}
...
\paragraph{} Les données d'ORANO nous fournissaient les pourcentages massiques en oxyde de chaque élément présent. Grâce à ces chiffres, nous avons calculé les concentrations en gramme par litre d'eau de chaque minéral retenu. Pour cela, nous avons introduit la densité de la roche ainsi que sa porosité et utilisé la formule : $[mineral] = \dfrac{pourcentage(oxyde)}{densite \times porosite} $ . Ainsi, on obtient le tableau 8 qui donne la concentration en minéraux des différents faciès.
...
L’objectif est de restaurer la qualité de l'eau dans les nappes aquifères telle qu'elle était avant l'exploitation minière. Bien que toutes les caractéristiques chimiques ne puissent pas être restaurées, l'eau doit être assurée aux mêmes utilisations qu'auparavant. %à reformuler (je trouve pas)
L'USAEC (\textit{United States Atomic Energy Commission}) formule des recommandations concernant la caractérisation du niveau de base de la qualité des eaux souterraines avant le début des opérations minières, la surveillance pour détecter l’évolution de lixiviat pendant l'exploitation minière et la surveillance à long terme pour déterminer à quel moment la qualité des eaux souterraines se stabilise après la fermeture des opérations minières.
...
%Dans le site Zamzow, 0,171~milligramme par litre d'uranium correspond à la valeur moyenne la plus élevée de cette zone. %pas utile àmha
Cependant, une étude publiée par l'\textit{U.S. Geological Survey} en 2009 a révélé que «~À ce jour, aucune opération après ISR (in-situ recovery) aux États-Unis n'a réussi à restaurer l'eau dans l'aquifère au niveau de base~».
...
%Plusieurs méthodes de restauration sont décrites par l'\textit{Intercontinental Energy Corporation}. Les deux les plus courantes sont le balayage des eaux souterraines (\textit{groundwater sweep}) et l'osmose inversée (\textit{reverse osmosis}).
...
La première découverte d'uranium enregistrée au Canada a eu lieu dans les années 1930 dans les Territoires du Nord-Ouest. %Pendant de nombreuses années, le Canada a été le plus grand exportateur de minerai d'uranium au monde. 
...
\lstinputlisting[language=Python, caption= Variation du facteur d'émanation selon le diamètre des grains ]{emanation.py}
% LJ, vous devrez envoyer le rapport PDF à Sabine. les encadrants en copie
% sabine.cantournet@mines-paristech.fr 
% vous avez son mail ?? ça marche mais je ne sais pas qui c'est
% noté ! oui le TeX ne va pas l'intéresser je pense =)

% j'ai encore la microcline a rajouter je le fais tout de suite
\newpage
...
\end{figure}
%LJ-rate-Microcline-timestep0.png


\newpage
La dissolution de microcline et la précipitation de montmorillonite peuvent être observées dès le début de la simulation. Ces phénomènes conjoints impliquent une baisse du pH du milieu.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{LJ-rate-Microcline-timestep0.png}
    \caption{Vitesse de réaction de la microcline à l'état initial}
    \label{fig:rate_micro_0}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{LJ-rate-Montmorillonite-timestep0.png}
    \caption{Vitesse de réaction de la montmorillonite à l'état initial}
    \label{fig:rate_}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{LJ-rate-Microcline-timestep1.png}
    \caption{Vitesse de réaction de la microcline sous régime de pluies fortes au début de la simulation}
    \label{fig:rate_}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{LJ-rate-Montmorillonite-timestep1.png}
    \caption{Vitesse de réaction de la montmorillonite sous régime de pluies fortes au début de la simulation}
    \label{fig:rate_}
\end{figure}
