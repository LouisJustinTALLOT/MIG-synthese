...
\usepackage{array}
\usepackage{listings}
\usepackage[amssymb]{SIunits} %je crois qu'il est incompatible avec certains package (amssymb ?) mais il serait pourtant bien pratique. Une idée ? 
% je le trouve pas méga utile perso... exemples ?
% C'était pour les unités, mais y a peut-être moyen d'en mettre un autre... (je m'y connais pas assez) --Sophian
% mais il va pas en avoir tant que ça si ?
% un $\text{m}^3$ ça marche bien hein
%non ça met le m en italique (détail j'avoue xD)
%yep bien sûr, comme tu veux
...
% euh si si j'ai pas encore cherché comment c'est tout -LJ
% c'est faaaaaaaaaaaaait !!!
% Super !! :D --Sophian
...
\lstset{numbers=left, stepnumber=5, firstnumber=1, breaklines = true,backgroundcolor=\color{grey!10!}}%, numbersep=5pt}
...
\begin{itemize}
    \item Camille \textsc{CHAUTARD} et Michael \textsc{DESCOSTES} (Orano Mining, R\&D Env Dpt) \includegraphics[width = 40pt]{oranologo.png}
\end{itemize}
...
%Dsl ^^ c'était Sophian. Merci !! AH mais ne les range pas comme annexe, c'est pas une annexe normalement je crois, faut les mettre à la fin du rapport
