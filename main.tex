...
\author{Elèves des Mines de Paris \\ 
...
\\ Encadrants du Centre de Géosciences (Mines ParisTech) \\
Irina \textsc{Sin}, 
...
\paragraph{} Les résidus de traitement, dont on a extrait l’uranium, contiennent encore des traces d’uranium et de radium, qui se situe dans la chaîne de désintégration radioactive de l’uranium (figure II-A2-1).

%Wikipédia
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{II_A2_1.png}
    \caption{Chaîne de désintégration de l’uranium}
    \label{fig:desintegration_uranium}
\end{figure}

Au sein d’un grain de résidu, les atomes de radium, dont la demi-vie est de 1602 années, se désintègrent en atomes de radon en libérant une particule alpha. Alors que la radioactivité alpha de l’uranium ou du radium, libérée en profondeur, ne pose pas de risque radiologique direct, le radon naît à l’état gazeux et peut atteindre la surface.

Il est donc impératif de savoir quelle proportion de radon est transportée vers la surface, pour évaluer les risques et prendre les mesures de sûreté nécessaires.

La première étape du transport du radon est appelée émanation. Le radium se désintègre au sein des résidus de traitement, qui sont sous forme de grains, et ainsi, tous les atomes de radon ne parviennent pas à atteindre les porosités. En effet, la distance $R_s$ que les atomes peuvent parcourir de l’ordre de 30 nm dans un minéral de densité commune. Cette distance est de 50 $\mu$m dans l’air et de 50 nm dans l’eau. %coucou guillaume 
% si tu as du code python on le mettra en annexe !! :)

On définit alors le facteur d’émanation comme étant la proportion d’atomes de radon qui atteignent les porosités sans être absorbés par un autre grain. Il est possible d’estimer le facteur d’émanation $F_e$ \cite{fleischer_theory_1983} dans le cas simplifié d’un grain sphérique de diamètre D, sans prendre en compte l’humidité ou la potentielle absorption des atomes par un grain adjacent. La formule suivante est donnée en fonction du paramètre $x=2\,R_s/D$ et on a tracé la fonction selon le diamètre, avec $R_s=30$ nm dans le grain (figure \ref{fig:facteur_emanation}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{II_A2_2.png}
    \caption{Facteur d’émanation en fonction du diamètre des grains}
    \label{fig:facteur_emanation}
\end{figure}

La littérature scientifique permet de confirmer l’ordre de grandeur obtenu par le calcul simplifié pour des grains dont le diamètre est de l’ordre du micromètre (figure \ref{fig:facteur_emanation_valeurs}) \cite{ferry_migration_2000}. On note que le facteur d’émanation est d’autant plus élevé que les résidus sont humides puisque l’eau, avec une distance d’arrêt plus courte, empêche les atomes d’être absorbés par les grains adjacents.

\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{II_A2_3.png}
    \caption{Quelques valeurs de facteurs d’émanations (Ferry C. 2000)}
    \label{fig:facteur_emanation_valeurs}
\end{figure}
...
\paragraph{} Au sein des résidus, le radium se désintègre en radon, mais le radon se désintègre également, plus rapidement. Au bout d’un certain temps, autant de radon est créé que désintégré. La concentration de radon dans les résidus est alors constante, c’est l’équilibre séculaire. Déterminons l'activité volumique du radon ($Bq/m^3$) dans les résidus atteinte à cet équilibre.

On se place dans les résidus, de concentration en radium solide $c_{Ra}$ constante ($mol/m^3$). La proportion de moles de $Rn(g)$ créés par désintégration atteignant les porosités par unité de volume est $E c_{Ra}$, où $E$ est le facteur d’émanation. On note $\lambda_{Ra}$ et $\lambda_{Rn}$ les coefficients de désintégration du radium et du radon en $s^{-1}$. On caractérise le milieu par une masse volumique $\rho$, une porosité $\omega$ et une saturation en gaz $S_g$. La concentration en radon $c_{Rn}$ en $mol/m^3$ évolue selon l'équation :

$$
\frac{d \omega S_g c_{Rn}}{dt} = - \lambda_{Rn} \omega S_g c_{Rn} + \lambda_{Ra} E (1-\omega) c_{Ra}
$$
...
Si la diffusion se fait majoritairement vers la surface, l’advection se fait le plus souvent dans l’autre sens, avec les infiltrations dues aux précipitations. Ainsi, travailler sur un modèle purement diffusif ne sous-estime pas le transport du radon vers la surface, mais peut légèrement le surestimer.
...
\paragraph{Estimation du facteur $r$}
...
$$
d_{Gy}=\frac{E_{absorbée}}{m}=\frac{e_{\alpha} V \Delta t}{m}=\frac{FfV\Delta t}{m}a
$$
...
$$
r'= \omega_{rad} \omega_{tissu}  fV/m=6,1 \; .10^{-6} \; mSv/(Bq/m^3.h)
$$
...
La détermination du temps de résidence de l'eau dans le système peut se faire de différentes façons. Une approximation linéaire permet un calcul d’ordre de grandeur : une modélisation par deux zones, une zone de résidus et une de granite fracturé, donne un temps de résidence d’environ 15 ans. La solution analytique donne également le même ordre de grandeur, avec un temps de résidence entre 20 et 22 ans de l'eau dans le système. Enfin, le modèle hydrogéologique montre l’évolution du traceur des résidus : on obtient ici un temps de résidence de 25 ans.
...
\paragraph{} En régime stationnaire, l’équation différentielle admet la solution :
$$
c(z)=A \exp(-\frac{z}{L})+B \exp (\frac{z}{L})
$$
La constante $B$ est nulle, car la concentration est nulle à l’infini. On place l’interface résidus/stériles à la cote $z=0$. L’équilibre séculaire dans les résidus implique alors l’égalité $c(0)=A=c_0$.
$$
c(z)=c_0 \exp(-\frac{z}{L})
$$

Par ailleurs, notons $erfc$ la fonction d’erreur complémentaire :
$$
erfc(u)=\frac{2}{\sqrt{\pi}} \int_{u}^{+\infty } e^{-\xi^2} d\xi
$$
Il existe une solution de l’équation en régime transitoire avec $c(0,t)=c_0$, de la forme :
$$
c(z,t)=\frac{c_0}{2} \Big( \exp(-\frac{z}{L}) \, erfc(\frac{z}{2L\sqrt{ \lambda t}} - \sqrt{\lambda t}) + \exp(\frac{z}{L}) \, erfc(\frac{z}{2L\sqrt{\lambda t}} + \sqrt{\lambda t} ) )
$$
L’expression admet un temps caractéristique de convergence vers l’état stationnaire :
$$
\tau=1/\lambda=4,8 \; .10^5 \; s=5j \; 13h
$$
...
\paragraph{} Dans un bâtiment habité, le niveau de référence d’activité volumique est de $a_n=300 \; Bq/m^3$. Imaginons que l’on souhaite respecter cette valeur à la surface de l’endroit où sont enfouis les résidus. Le facteur d’atténuation minimum requis est alors :
$$
...
La figure suivante montre l'impact significatif que le coefficient de diffusion a sur la diffusion du radon. Sur le site de la Ribière, on peut l'estimer autour de $1 \; .10^5 \; m^2s^{-1}$.

\begin{figure}[H]
    \centering
    \includegraphics[width = 9 cm]{III_C_8.png}
    \caption{Fonction $\exp(-z/L)$ en fonction du coefficient de diffusion}
    \label{fig:sens_diffusion}
\end{figure}

Il est important d'estimer le coefficient de diffusion des stériles avec précision, dans le but de déterminer l'épaisseur minimale de la couche à déposer au-dessus des résidus.
...
% Paul ?
(juste expliquer l'importance des modèles numériques par rapport au modèle analytique)

\paragraph{Deux modèles numériques}
\paragraph{} Présenter rapidement : Modèle Nicolas. Modèle Irina. Modèle Nicolas/Paul. + donner code en annexe (j'ai déjà mis deux codes type Irina)
% Paul ?

\paragraph{Relation entre fugacité et activité du radon}
...
$$
P_{Rn} (z,t)=\frac{n_{Rn} RT}{V}=RTc(z,t)=\frac{RT}{N_a \lambda} a(z,t)
$$
...
La figure suivante donne, en pourcentage, l'atténuation du radon en fonction des coefficients de Millington-Quirk sélectionnés. La simulation a été réalisée sur un modèle de 2 mètres, réduit par rapport à la réalité du site, mais cela suffit pour visualiser la sensibilité.

\begin{figure}[H]
    \centering
    \includegraphics[width = 10 cm]{III_C_10.png}
    \caption{Résultats HYTEC en fonction des coefficients de Millington-Quirk}
    \label{fig:sens_hytec_mq}
\end{figure}

Les figures suivantes illustrent la sensibilité des paramètres de Van-Genuchten. Ces coefficients interviennent dans l’expression de la pression capillaire, qui joue un rôle lorsque la saturation en eau n’est pas uniforme, en particulier dans un modèle vertical. On a :
$$
p_c (S_e)= \frac{1}{\alpha} ( {S_{e,eff} }^{\frac{n}{n-1}} -1 )^{1/n}
$$
Où on a posé $S_{e,eff}$ la saturation effective en eau. Étudions la sensibilité des paramètres $\alpha$ et $n$ à l’aide d’un modèle vertical HYTEC.

La figure suivante montre qu'une variation d’ordre $\pm 1$ du paramètre $\alpha$ ne modifie pas l’ordre de grandeur de la fugacité à la surface. En effet, à $n=1,56$ fixé, pour $\alpha \in \left\{2 ; \; 3,6 ; \; 4 ; \; 5\right\}$, on obtient :

\begin{figure}[H]
    \centering
    \includegraphics[width = 10 cm]{III_C_11.png}
    \caption{Résultats HYTEC en fonction de $\alpha$}
    \label{fig:sens_hytec_alpha}
\end{figure}

La figure suivante montre qu'une variation d’ordre $\pm 0,1$ du paramètre $n$ ne modifie pas l’ordre de grandeur de la fugacité à la surface. En effet, à $\alpha=3,6$ fixé, pour $n \in \left\{1,30 ;\;1,56  ;\;1,70;\;2\right\}$, on obtient :
 
\begin{figure}[H]
    \centering
    \includegraphics[width = 10 cm]{III_C_12.png}
    \caption{Résultats HYTEC en fonction de $n$}
...
\paragraph{} Premièrement, le modèle analytique permet d'obtenir des ordres de grandeur. Pour un coefficient de diffusion moyen $D=1,0 \; .10^{-5} \; m^2/s^$, une porosité moyenne $\omega=0,30$ et des coefficients de Millington-Quirk égaux à 2, le tableau suivant donne les longueurs caractéristiques et les épaisseurs de stériles nécessaires pour respecter le niveau de référence de$300\; Bq/m^3$, en fonction de la saturation en gaz. Plus il y d’eau dans les pores, plus la saturation en gaz est faible.
...
Lorsque la roche est saturée, la diffusion est moindre. Au contraire, une période sèche favorise la diffusion. Avec une épaisseur de stériles d'environ $6,5\; m$, le site de la Ribière peut être exposé à des activités de radon plus élevées en été quand il pleut moins souvent.
...
\paragraph{} Nous utilisons ici le modèle numérique 2. Avec des précipitations moyennes de $400\;mm/an$ et une simulation pendant un an, on obtient un sol saturé après les précipitations (gauche). Sans précipitations, la zone non-saturée a un profil graduel (droite).

\begin{figure}[H]
    \centering
    \includegraphics[width = 10 cm]{III_C_5.png}
    \caption{Teneur en eau avec ou sans précipitations}
    \label{fig:water_content_rain}
\end{figure}

La figure suivante donne la fugacité du radon en fonction de la cote (où l'origine est au niveau des résidus), en échelle logarithmique. La courbe bleue correspond au cas pluvieux et la courbe brune au cas témoin.

\begin{figure}[H]
    \centering
    \includegraphics[width = 12 cm]{III_C_6.png}
    \caption{Fugacité du radon avec ou sans précipitations}
    \label{fig:rain_fugacity}
\end{figure}

L’échelle logarithmique montre que la concentration en radon est divisée par un facteur 10. On en déduit que les précipitations sont bénéfiques quant à l’atténuation du radon gazeux. Le phénomène en jeu est la dissolution du radon dans l’eau. La présence d’eau augmente la pression, donc déplace l’équilibre de Henry entre le radon gazeux et le radon aqueux, dont la concentration augmente dans la zone humide. La figure suivante donne les profils de pression, de fugacité du radon gazeux, de concentration en radon aqueux et de radon gazeux alors que le sol est en train de se remplir d'eau. Les courbes rouges correspondent au profil sec, les courbes bleues au cas pluvieux.

\begin{figure}[H]
    \centering
    \includegraphics[width = 12 cm]{III_C_7.png}
    \caption{Dissolution du radon dans l'eau de pluie}
    \label{fig:dissolution_radon_pluie}
\end{figure}
...
\paragraph{} Les stériles contiennent du radium d’activité massique $A_{Ra,s}$. Le radium se désintègre avec un coefficient $\lambda_{Ra}$. Ajoutons un terme source dans l’équation différentielle en concentration de radon $c(z,t)$ ($mol/m^3$) :
$$
...
$$
\frac{d^2 c}{dz^2} - \frac{c}{L^2}= \frac{\lambda_{Ra}(1-\omega)E}{D_{eff}} c_{Ra}
$$
...
$$
R=\frac{c_s}{c_0}= \frac{\frac{(1-\omega)E\rho}{\lambda_{Rn}\omega S_g N_a} A_{Ra,s}}{\frac{ E_r (1-\omega_r) \rho_r }{\lambda_{Rn} \omega_r S_{g,r} N_a} A_{Ra,r}} = \frac{E}{E_r} \frac{(1-1/\omega)}{(1-1/\omega_r)} \frac{\rho}{\rho_r} \frac{S_{g,r}}{S_g} \frac{A_{Ra,s}}{A_{Ra,r}}
$$
On a alors :
$$
...
\begin{itemize}
  ...
  \item Sinon, la production de radon dans les stériles peut être visible.
...
\begin{itemize}
  ...
  \item Stériles miniers $A_{Ra,s}= 0 - 0,04 - 0,4 \; Bq/g$, résidus $A_{Ra,s}= 4,0 \; Bq/g$
\end{itemize}
On obtient $ R=7,0\%$, ce qui signifie la production de radon dans les stériles, même le radium y est environ trois fois moins actif, a une influence sur la courbe de diffusion du radon. On peut ainsi comparer les concentrations à la surface, avec ou sans terme source ($L=0,6\;m$, $h=6,5\;m$):
\begin{itemize}
  \item Pour $A_{Ra,s}=0 \; Bq/g$ : $c(h) = 1,4 \; .10^{-16} mol/m^3$ soit $180 \; Bq/m^3$
  \item Pour $A_{Ra,s}=0,04 \; Bq/g$ : $c(h) = 3,0 \; .10^{-16} mol/m^3$ soit $373 \; Bq/m^3$
  \item Pour $A_{Ra,s}=0,4 \; Bq/g$ : $c(h) = 1,7 \; .10^{-15} mol/m^3$ soit $2,1 \; .10^3 Bq/m^3$
\end{itemize}
...
\newpage
\lstinputlisting[language=hytec, caption=robin1]{ribieresable.htc}
\newpage
\lstinputlisting[language=hytec, caption=robin2]{ribieresablealumax.htc}



\newpage
\section{Modèle topographique du site}
\lstinputlisting[language=C++,caption = Modèle topographique du site de La Ribière ]{ribiere.geo}
